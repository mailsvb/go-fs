...
// is not enough, because a session is what the account established through
// the login form on purpose, while a header is what a program or a browser
// sends along with everything else.
...
func (s *Server) handleAdmin(set *settings, w http.ResponseWriter, r *http.Request) {
	if s.admin == nil || !set.cfg.EnableAdminInterface {
		...
		s.log.Debug("http admin interface is not enabled", "address", addressOf(r))
		http.NotFound(w, r)
		return
	}
	...
	if !cred.user.isAdmin {
		s.log.Warn("http admin interface refused, the account is not an admin",
			"user", cred.user.name, "address", addressOf(r))
	...
	if r.Method == http.MethodPost && !s.sameSite(w, r) {
		return
	}

	s.log.Info("http admin interface request", "user", cred.user.name,
		"action", r.URL.Query().Get(sessionParam), "method", r.Method, "address", addressOf(r))
